...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'YourModuleName' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\YourModuleName" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the YourModuleName module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\YourModuleName"
